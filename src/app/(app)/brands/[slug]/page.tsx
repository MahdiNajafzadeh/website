  ...
  return (
    <div className="mx-auto max-w-[1440px] px-4 py-8 md:px-8">
      <nav className="mb-6 flex items-center gap-1.5 text-sm text-[#707072] dark:text-[#9e9ea0]" aria-label="Breadcrumb">
        ...
        <Link href="/brands" className="hover:text-[#111111] dark:hover:text-white">{t('common.brands')}</Link>
        <span aria-hidden>/</span>
      ...
      <div className="flex items-center gap-4 rounded-[30px] bg-[#f5f5f5] dark:bg-[#1a1a1a] p-6">
        {iconUrl ? (
          ...
          <div className="flex size-16 items-center justify-center rounded-full bg-white text-[20px] font-medium text-[#111111] ring-1 ring-[#e5e5e5]">{brand.name.charAt(0).toUpperCase()}</div>
        )}
        <div>
          <h1 className="text-[32px] font-medium leading-[1.2] text-[#111111] dark:text-white">{brand.name}</h1>
          {brand.description && <p className="mt-1 text-[14px] font-medium text-[#707072] dark:text-[#9e9ea0]">{brand.description}</p>}
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Link href={`/products?brand=${brand.slug}`} className="rounded-full bg-[#111111] px-4 py-1.5 text-[14px] font-medium text-white hover:bg-[#111111]/90 dark:bg-white dark:text-[#111111]">
          {t('common.productsCount')} {brand.name}
        </Link>
        <Link href="/brands" className="rounded-full bg-white px-4 py-1.5 text-[14px] font-medium text-[#111111] ring-1 ring-[#cacacb] hover:bg-[#f5f5f5] dark:bg-transparent dark:text-white dark:ring-[#39393b]">{t('common.all')}</Link>
      </div>
      ...
      {products.length === 0 ? (
        <div className="mt-8 rounded-[30px] bg-white p-12 text-center ring-1 ring-[#e5e5e5] dark:bg-[#1a1a1a] dark:ring-[#39393b]">
          ...
          <Link href="/products" className="mt-4 inline-flex rounded-full bg-[#111111] px-6 py-2 text-[14px] font-medium text-white dark:bg-white dark:text-[#111111]">{t('common.viewAll')}</Link>
        </div>
      ) : (
                ...
                <Card className="overflow-hidden rounded-[30px] border border-[#e5e5e5] bg-white dark:border-[#39393b] dark:bg-[#1a1a1a] p-0 gap-0">
                  ...
                  <CardContent className="flex flex-col gap-1.5 p-4">
                    <div className="flex items-center gap-1.5">
                      {isOutOfStock && <Badge variant="outline" className="rounded-full border-[#cacacb] bg-white text-xs text-[#707072] dark:border-[#39393b] dark:bg-transparent dark:text-[#9e9ea0]">{t('common.outOfStock')}</Badge>}
                      {categoryName && <Badge variant="secondary" className="rounded-full bg-[#f5f5f5] dark:bg-[#39393b] dark:text-white text-xs text-[#111111]">{categoryName}</Badge>}
                    </div>
                    <h3 className="line-clamp-2 text-[16px] font-medium leading-[1.5] text-[#111111] dark:text-white">{product.name}</h3>
                    <p className="text-[14px] font-medium text-[#111111] dark:text-white">{price === 0 ? t('common.contactForPrice') : `${price.toLocaleString('fa-IR')} ${t('common.toman')}`}</p>
                  </CardContent>
                </Card>
              </Link>
            )
