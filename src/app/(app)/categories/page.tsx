...
function buildPageHref(base: SearchParams & { page?: number | string }, overrides: Partial<Record<string, string | undefined>>) {
...
export default async function CategoriesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-4 py-8 md:px-8">
      ...
      <h1 className="text-[32px] font-medium leading-[1.2] text-[#111111] dark:text-white">{t('categories.title')}</h1>
      ...
      <form action="/categories" method="get" className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex flex-1 items-center gap-2">
          ...
          />
          <button type="submit" className="inline-flex h-10 items-center justify-center rounded-full bg-[#111111] px-6 text-[14px] font-medium text-white hover:bg-[#111111]/90 dark:bg-white dark:text-[#111111] dark:hover:bg-[#e5e5e5]">
            {t('products.searchAction')}
          </button>
        </div>
        {q && (
          <Link href="/categories" className="text-[14px] font-medium text-[#707072] underline hover:text-[#111111] dark:text-[#9e9ea0] dark:hover:text-white">
            {t('products.clearAll')}
          </Link>
        )}
      </form>
      ...
      {categories.length === 0 ? (
        <div className="mt-8 rounded-[30px] bg-[#f5f5f5] p-12 text-center dark:bg-[#1a1a1a]">
          <p className="text-[16px] font-medium text-[#111111] dark:text-white">{t('categories.noCategories')}</p>
            ...
            <Link key={cat.id} href={`/products?category=${cat.slug}`} className="group">
              <Card className="overflow-hidden rounded-[30px] border border-[#e5e5e5] p-0 hover:border-[#cacacb] dark:border-[#39393b] dark:bg-[#1a1a1a] transition-colors">
                <div className="flex aspect-[4/3] flex-col items-center justify-center gap-3 bg-[#f5f5f5] p-6 dark:bg-[#111111]">
                  <div className="flex size-16 items-center justify-center rounded-full bg-white text-[20px] font-medium text-[#111111] ring-1 ring-[#e5e5e5]">{cat.name.charAt(0).toUpperCase()}</div>
                  <span className="line-clamp-2 text-center text-[16px] font-medium leading-[1.5] text-[#111111] dark:text-white">{cat.name}</span>
                  {cat.description && <span className="line-clamp-2 text-center text-[12px] font-medium text-[#707072] dark:text-[#9e9ea0]">{cat.description}</span>}
                </div>
                <CardContent className="p-3 text-center">
                  <span className="text-[12px] font-medium text-[#707072] group-hover:text-[#111111] dark:text-[#9e9ea0] dark:group-hover:text-white">{t('categories.viewProducts')}</span>
                </CardContent>
              </Card>
            </Link>
          ))}
        ...
        <div className="mt-8">
          <Pagination>
            <PaginationContent>
              {page > 1 && (
                <PaginationItem>
                  <PaginationPrevious href={buildPageHref({ q, page: String(page) }, { page: String(page - 1) })} />
                </PaginationItem>
              )}
                ...
                .map((p, idx) =>
                  p === 'ellipsis' ? (
                    ...
                    <PaginationItem key={p}>
                      <PaginationLink href={buildPageHref({ q, page: String(page) }, { page: String(p) })} isActive={p === page}>
                        {p}
                      </PaginationLink>
                    </PaginationItem>
                  ),
              ...
              {page < totalPages && (
                <PaginationItem>
                  <PaginationNext href={buildPageHref({ q, page: String(page) }, { page: String(page + 1) })} />
                </PaginationItem>
              )}
            </PaginationContent>
          </Pagination>
          <p className="mt-3 text-center text-[12px] font-medium text-[#707072] dark:text-[#9e9ea0]">
            {tFmt('categories.pagination', { page: page.toLocaleString('fa-IR'), totalPages: totalPages.toLocaleString('fa-IR'), count: totalDocs.toLocaleString('fa-IR') })}
          </p>
        </div>
      )}
    </div>
