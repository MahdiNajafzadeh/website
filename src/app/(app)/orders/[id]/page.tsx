...
async function fetchOrder(id: number, userId: number): Promise<Order | null> {
  try {
    ...
    const ownerId =
      typeof order.customer === 'number' ? order.customer : (order.customer as User)?.id
            ...
            <CardContent className="p-6">
              ...
              <ul className="mt-4 divide-y divide-[#e5e5e5]">
                {(order.items ?? []).map((item, idx) => {
                  ...
                  return (
                    <li key={item.id ?? idx} className="flex items-center justify-between gap-4 py-3">
                      ...
                      <span
                        className={`shrink-0 text-[14px] font-medium ${
                          isZero ? 'text-[#d30005]' : 'text-[#111111]'
                        }`}
                      >
              ...
              <div className="mt-4 flex items-center justify-between border-t border-[#e5e5e5] pt-4">
                ...
                <span
                  className={`text-[16px] font-medium ${
                    order.hasZeroPrice ? 'text-[#d30005]' : 'text-[#111111]'
                  }`}
                >
                  ...
                  return (
                    <li key={s} className="flex items-center gap-2">
                      ...
                      {idx < STATUS_HISTORY.length - 1 ? (
                        <span aria-hidden className="text-[#9e9ea0]">›</span>
                      ) : null}
                    </li>
                  )
                })}
