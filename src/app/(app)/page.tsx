...
import type { Brand, Category, Product, SiteSetting } from "@/payload-types";
import { getCurrentUser } from "@/lib/current-user";
import { t } from "@/lib/t";
import { getPayloadClient } from "@/lib/payload";
import { getPrimary } from "@/lib/collections";
...
export default async function HomePage() {
    ...
    const siteName = (settings as SiteSetting | null)?.name ?? "فروشگاه";
    ...
    for (const p of allProductsForImage) {
        ...
        const url =
            getMediaUrl((p.images?.[0]?.image as never) ?? null) ??
            getMediaUrl(p.showcaseImage as never);
