...
async function fetchHomeData() {
  ...
  const [productsRes, popularRes, categoriesRes, brandsRes, settings] = await Promise.all([
    payload.find({ collection: 'products', where: VISIBLE_TRUE, sort: '-createdAt', limit: 4, depth: 1 }),
    payload.find({ collection: 'products', where: VISIBLE_TRUE, sort: '-inventory', limit: 3, depth: 1 }),
  ...
  const [categoryCounts, brandCounts] = await Promise.all([
    Promise.all(
      categories.map((c) =>
        payload.count({ collection: 'products', where: { visible: { equals: true }, category: { equals: c.id } } }),
      ),
    ),
    Promise.all(
      brands.map((b) =>
        payload.count({ collection: 'products', where: { visible: { equals: true }, brand: { equals: b.id } } }),
      ),
    ),
  ])
...
export default async function HomePage() {
  const [{ products, popular, productsTotal, categories, brands, categoryCounts, brandCounts, settings }, currentUser] =
    await Promise.all([fetchHomeData(), getCurrentUser()])

  const siteName = settings?.siteName?.fa || settings?.siteName?.en || 'فروشگاه'
  void getLogoUrl(settings?.logo)
  const totalInventory = popular.reduce((sum, p) => sum + (p.inventory ?? 0), 0) + products.reduce((sum, p) => sum + (p.inventory ?? 0), 0)
  const showPopular = popular.length > 0 && totalInventory > 0
  const primaryPhone = getPrimary(settings?.phones as unknown as { label?: string | null; number: string; isPrimary?: boolean | null }[] | null | undefined)
  ...
  for (const p of allProductsForImage) {
    ...
    const url = getMediaUrl((p.images?.[0]?.image as Media | number | null | undefined) ?? null) ?? getMediaUrl(p.showcaseImage as Media | number | null | undefined)
    ...
    <main id="content" className="bg-[#ffffff] dark:bg-[#111111]">
      ...
      >
        ...
        <div className="relative mx-auto flex max-w-[1440px] flex-col justify-end px-4 pb-10 pt-24 sm:px-6 sm:pb-14 sm:pt-28 lg:px-8 lg:pb-16 lg:pt-36">
          ...
          <h1
            ...
            style={{ fontFamily: '"Nike Futura ND", "Helvetica Now Display Medium", Helvetica, Arial, sans-serif' }}
          >
            {t('home.hero.title')}
          </h1>
          <p className="mt-4 max-w-[52ch] text-[16px] font-medium leading-[1.5] text-white/90">{t('home.hero.sub')}</p>
      ...
      <div className="sticky top-14 z-40 border-y border-[#e5e5e5] bg-[#f5f5f5] dark:border-[#39393b] dark:bg-[#1a1a1a]">
        <div className="mx-auto flex max-w-[1440px] items-center gap-2 overflow-x-auto px-4 py-2 text-[12px] font-medium text-[#111111] dark:text-white sm:px-6 lg:px-8">
          ...
          <span className="ml-auto hidden whitespace-nowrap text-[#707072] dark:text-[#9e9ea0] sm:inline"> {siteName}</span>
        </div>
      </div>

      {/* C — New Drops */}
      {products.length > 0 ? (
        <section className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8" data-od-id="new-drops" aria-labelledby="new-h">
          <SectionHead
        ...
        <section className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8" data-od-id="new-drops-empty">
          <Card className="rounded-none border border-dashed border-[#cacacb] bg-[#f5f5f5] p-0 dark:border-[#39393b] dark:bg-[#1a1a1a]">
            <CardContent className="p-10 text-center">
              ...
              <Link href="/products" className="mt-4 inline-flex h-9 items-center justify-center rounded-full bg-[#111111] px-6 text-[14px] font-medium text-white dark:bg-white dark:text-[#111111]">
                {t('common.viewAll')}
              </Link>
            </CardContent>
          </Card>
      ...
      {/* D — Category Tiles */}
      {categories.length > 0 ? (
        <section id="categories" className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8" data-od-id="categories" aria-labelledby="cat-h">
          <SectionHead
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {categories.slice(0, 6).map((cat, i) => (
              <CategoryCard key={cat.id} category={cat} count={categoryCounts[i] ?? 0} imageUrl={categoryImageMap.get(cat.id) ?? null} />
            ))}
          </div>
        </section>
      ) : null}
      ...
      {/* E — Contractor's Pick */}
      {showPopular ? (
        <section className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8" data-od-id="contractor-pick" aria-labelledby="popular-h">
          <SectionHead
      ...
      {/* F — Proof / Spec Strip */}
      <section className="border-y border-[#e5e5e5] bg-[#f5f5f5] py-10 dark:border-[#39393b] dark:bg-[#1a1a1a]" data-od-id="proof-strip">
        <div className="mx-auto grid max-w-[1440px] gap-8 px-4 sm:px-6 lg:grid-cols-3 lg:px-8">
          <div className="space-y-2">
            ...
            <p className="text-[12px] font-medium uppercase tracking-wide text-[#707072] dark:text-[#9e9ea0]">{t('home.proof.categoriesLabel')}</p>
            <p className="text-[14px] font-medium leading-[1.5] text-[#707072] dark:text-[#9e9ea0]">{t('home.brands.title')}</p>
          </div>
          <div className="space-y-2">
            <p className="text-[48px] font-medium leading-none tracking-[-0.02em] text-[#111111] dark:text-white">
              {brands.length.toLocaleString('fa-IR')}
            </p>
            <p className="text-[12px] font-medium uppercase tracking-wide text-[#707072] dark:text-[#9e9ea0]">{t('home.proof.brandsLabel')}</p>
            <p className="text-[14px] font-medium leading-[1.5] text-[#707072] dark:text-[#9e9ea0]">{t('home.proof.brandsLabel')}</p>
          </div>
          <div className="space-y-2">
            <p className="text-[48px] font-medium leading-none tracking-[-0.02em] text-[#111111] dark:text-white">
              {totalInventory.toLocaleString('fa-IR')}
            </p>
            <p className="text-[12px] font-medium uppercase tracking-wide text-[#707072] dark:text-[#9e9ea0]">{t('home.proof.stockLabel')}</p>
            <p className="text-[14px] font-medium leading-[1.5] text-[#111111] dark:text-white">
              {partnerDiscount > 0 ? tFmt('home.proof.procedure', { discount: partnerDiscount }) : 'قیمت صفر = استعلام. افزودن به سبد → بررسی → تایید → آماده‌سازی → تحویل.'}
            </p>
            <Link href="/about" className="inline-flex text-[14px] font-medium text-[#111111] underline hover:text-[#707072] dark:text-white dark:hover:text-[#9e9ea0]">
      ...
      {/* G — Brand Pills */}
      {brands.length > 0 ? (
        <section className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8" data-od-id="brands" aria-labelledby="brands-h">
          <SectionHead
      ...
      <section className="bg-[#111111] py-12 text-white dark:bg-[#111111] md:py-16" data-od-id="cta-band">
        <div className="mx-auto flex max-w-[1440px] flex-col gap-6 px-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="max-w-[28ch]">
            ...
            <p className="mt-3 text-[14px] font-medium leading-[1.5] text-white/80">{t('home.ctaBand.sub')}</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <Link href="/products" className="inline-flex h-12 items-center justify-center rounded-full bg-white px-8 text-[16px] font-medium text-[#111111] hover:bg-[#e5e5e5]">
              {t('home.ctaBand.primary')}
            </Link>
            <Link href="/contact" className="text-[14px] font-medium text-white/80 underline hover:text-white">
