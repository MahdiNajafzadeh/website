...
const waLink = (phone: string): string => `https://wa.me/98${digitsOf(phone).replace(/^0+/, "")}`;
    ...
    return (
        <div className="flex flex-col gap-1" dir="ltr">
            <div className="flex items-center gap-1">
                ...
                >
                    {copiedKey === "phone" ? <Check className="size-4 text-green-600" /> : <Copy className="size-4" />}
                </Button>
                {copiedKey === "phone" ? (
                    <span className="text-xs text-muted-foreground">{t("employee.customers.table.copied")}</span>
                ) : null}
            </div>
            {password !== null ? (
                <div className="mt-1 flex items-center gap-1">
                    ...
                    <Input dir="ltr" readOnly value={reveal ? password : "••••••••"} className="h-7 w-32 text-xs" />
                    <Button
                        type="button"
                        variant="ghost"
                        size="icon"
