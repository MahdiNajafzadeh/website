        ...
        <div className="flex items-center gap-2">
            ...
            {user ? (
                <DropdownMenu open={open} onOpenChange={setOpen}>
                    <DropdownMenuTrigger
                        render={
                            <Button variant="ghost" size="icon" aria-label={t("layout.header.account")}>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                        <div className="px-2 py-1.5 text-sm">
                            ...
                            <p className="text-xs text-muted-foreground">{t(`layout.role.${roleKey(user.role)}`)}</p>
                        </div>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={go("/account")}>{t("layout.nav.account")}</DropdownMenuItem>
                        <DropdownMenuItem onClick={go("/orders")}>{t("layout.nav.orders")}</DropdownMenuItem>
                        {user.role !== "customer" ? (
                        ...
                        <DropdownMenuItem onClick={handleLogout} disabled={loggingOut}>
                            ...
                            {loggingOut ? t("layout.header.loggingOut") : t("layout.header.logout")}
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            ) : (
