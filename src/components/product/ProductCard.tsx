...
export const ProductCard = async ({ product, locale, priority = false }: Props) => {
    ...
    const brandName =
        product.brand && typeof product.brand !== "number" ? localizedValue(product.brand.name, locale) : "";
    ...
    return (
        <Card className="group h-full overflow-hidden pt-0">
            <div className="relative aspect-square overflow-hidden bg-muted">
                ...
                {product.featured ? <Badge className="absolute end-2 top-2">{t("product.card.featured")}</Badge> : null}
                {isOut ? (
            ...
            <CardContent className="flex flex-col gap-2 p-4">
                ...
                <div className="mt-auto flex items-center justify-between pt-1">
                    {hidePrice ? (
                        <span className="text-sm text-muted-foreground">{t("product.price.callForPrice")}</span>
                    ) : (
