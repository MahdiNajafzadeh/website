...
export const addToCart = (item: Omit<CartItem, "quantity"> & { quantity?: number }): CartItem[] => {
    ...
    let next: CartItem[];
    if (existing) {
        next = items.map((i) => (i.productId === item.productId ? { ...i, quantity: i.quantity + quantity } : i));
    } else {
...
export const cartTotal = (items: CartItem[]): number => items.reduce((sum, i) => sum + i.price * i.quantity, 0);
