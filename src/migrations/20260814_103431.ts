...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `);
    await db.run(
        sql`CREATE UNIQUE INDEX \`brands_locales_locale_parent_id_unique\` ON \`brands_locales\` (\`_locale\`,\`_parent_id\`);`,
    );
  ...
  `);
    await db.run(
        sql`CREATE UNIQUE INDEX \`products_locales_locale_parent_id_unique\` ON \`products_locales\` (\`_locale\`,\`_parent_id\`);`,
    );
  ...
  `);
    await db.run(
        sql`CREATE INDEX \`site_settings_contact_info_phones_order_idx\` ON \`site_settings_contact_info_phones\` (\`_order\`);`,
    );
    await db.run(
        sql`CREATE INDEX \`site_settings_contact_info_phones_parent_id_idx\` ON \`site_settings_contact_info_phones\` (\`_parent_id\`);`,
    );
  ...
  `);
    await db.run(
        sql`CREATE INDEX \`site_settings_contact_info_emails_order_idx\` ON \`site_settings_contact_info_emails\` (\`_order\`);`,
    );
    await db.run(
        sql`CREATE INDEX \`site_settings_contact_info_emails_parent_id_idx\` ON \`site_settings_contact_info_emails\` (\`_parent_id\`);`,
    );
  ...
  `);
    await db.run(
        sql`CREATE INDEX \`site_settings_contact_info_addresses_order_idx\` ON \`site_settings_contact_info_addresses\` (\`_order\`);`,
    );
    await db.run(
        sql`CREATE INDEX \`site_settings_contact_info_addresses_parent_id_idx\` ON \`site_settings_contact_info_addresses\` (\`_parent_id\`);`,
    );
  ...
  `);
    await db.run(
        sql`CREATE INDEX \`site_settings_brand_logos_order_idx\` ON \`site_settings_brand_logos\` (\`_order\`);`,
    );
    await db.run(
        sql`CREATE INDEX \`site_settings_brand_logos_parent_id_idx\` ON \`site_settings_brand_logos\` (\`_parent_id\`);`,
    );
    await db.run(
        sql`CREATE INDEX \`site_settings_brand_logos_image_idx\` ON \`site_settings_brand_logos\` (\`image_id\`);`,
    );
  ...
  `);
    await db.run(
        sql`CREATE UNIQUE INDEX \`site_settings_locales_locale_parent_id_unique\` ON \`site_settings_locales\` (\`_locale\`,\`_parent_id\`);`,
    );
}
